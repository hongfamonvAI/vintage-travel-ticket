...
3. **Romanization/number** — when the sampled language mode permits a Roman destination line, set it mechanically, geometrically, scientifically, or in a period serif. Otherwise use a serial, value, date, seal code, or route number for this role. It must feel separately typeset.

Every output must make all three roles visibly different when all three are present. A sparse ticket may omit role 2 or 3, but may never use one generic Song face for every visible field. Do not invent an English line merely to demonstrate role 3.
...
| T02 formal band | centered white Song/Ming or squared sans knockout | tiny condensed neutral labels | geometric uppercase Roman line **or mono serial when Chinese-only** | title lives inside a dominant colour band with generous top/bottom rules |
...
| T09 Republican book-ticket | Zhuque-Fangsong-like tall title, horizontal or vertical | small Song/Ming lead type | thin serif Romanization when allowed; otherwise stamped code/number only | narrow rail, framed cartouche, or literary souvenir strip |
...
| T11 art-deco souvenir | medallion, geometric display, or tall condensed title | delicate narrow Fangsong | geometric bilingual footer when allowed, or geometric code footer when Chinese-only | saturated dark card, metallic line, coloured terminal band |
...
| T13 museum portrait | small seal/display title above or below portrait | compact serif museum line | widely spaced uppercase museum name when allowed, or widely spaced serial | portrait frame and footer establish hierarchy without requiring bilingual text |
...
- Only fit a Roman title when `language_mode` permits one. For allowed Roman titles longer than 18 characters, use a narrow uppercase line, split at a natural hyphen/space, or place the Roman line in a footer. Do not let it compete with the Chinese title.
